...
/**
 * Crazydb 主题修改密码页（auth-card 卡片表单，风格与登录/注册一致）。
 *
...
<div class="auth-card">
    ...
    <form class="auth-form" method="post" action="<?= Html::encode($urlGenerator->generate('user/modify-password')) ?>">
        ...
        <label>旧密码：<input type="password" name="old_password" required autofocus autocomplete="current-password"></label>
        <label>新密码（8-20 字符）：<input type="password" name="password" required autocomplete="new-password"></label>
        <label>确认新密码：<input type="password" name="password_repeat" required autocomplete="new-password"></label>
        <button type="submit">确认修改</button>
    </form>
    <p class="auth-tip">修改成功后需使用新密码重新登录（所有设备的登录状态与记住我均失效）。</p>
</div>
