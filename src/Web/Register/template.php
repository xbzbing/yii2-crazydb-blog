...
<div class="auth-card">
    ...
    <?php else: ?>
        <form class="auth-form" method="post" action="<?= Html::encode($urlGenerator->generate('site/register')) ?>">
            ...
            <?php endforeach; ?>
            <label>用户名（最多 20 字符）：<input type="text" name="username" required></label>
            <label>昵称（最多 80 字符）：<input type="text" name="nickname" required></label>
            ...
            <label>个人网站（可选）：<input type="text" name="website" placeholder="http:// 或 https://"></label>
            <label>个人简介（可选）：<textarea name="info" rows="3"></textarea></label>
            <label>验证码：
